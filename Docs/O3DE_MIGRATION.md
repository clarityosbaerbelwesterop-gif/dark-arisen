...
| Credits | `ContentSource/Story/Credits/CreditsAuthority.json` | content build | IMPLEMENTED: `L_Credits` with a factual roll (`verifiedTechnologyByEngine.O3DE`, provenance only), starts only after The Wake After (`CreditsComponent`, VERIFIED in the story probe); the roll is drawn by the front end (75 s, skippable after 8 s, then `L_FrontEnd`, as `ACreditsPresentationActor`), VERIFIED through a captured `FontDrawInterface` |
| Front end: main menu, pause, settings, subtitles, documents | `UI/AlphaMenuPlayerController`, `AlphaMenuHUD`, `AlphaGameplayPlayerController`, `PostureOnlyHUD` | presentation | IMPLEMENTED: Core `FrontEndMenu`, `HudModel`, `CreditsRoll` and `UserSettings`; Gem `FrontEndSystemComponent` (keyboard and gamepad, text through `AzFramework::FontDrawInterface`) and `MainMenuComponent` in the materialised `L_FrontEnd`, which `autoexec.cfg` loads first. Main menu: New Game, Continue (the newer of autosave and manual save), Settings, Quit. Pause: Resume, Save, Load, Settings, Main Menu; simulation tick scale 0 while paused; releases still reach gameplay so no key sticks. Settings persist under `@user@/DarkArisen/settings.cfg` and apply VSync, window mode and resolution through O3DE console variables and the window bus. Ray tracing is offered only after a measured frame time fits the budget (no measurement tool yet, so it stays off); no upscaler is offered because O3DE 2605.0 integrates none. The graphics preset is stored and shown, but no Atom feature consumes it until it can be measured on a GPU (PARTIAL). No minimap and no world map (owner authority pending). VERIFIED in the story probe: credits → front end → settings → New Game → pause → save → resume. Visual layout RUNTIME VERIFY PENDING |

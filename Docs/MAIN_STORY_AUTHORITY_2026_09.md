...
| ID | Trigger | Participants / objects | Route | Required playable beats | Completion | Persistent facts | Presentation |
|---|---|---|---|---|---|---|---|
| `Main.C04.01.ClosedHarbor` | WorldState | Jake, crew, Imperial/Albion/Liberation actors | sea route → blockaded harbor | discover the next Ethan lead is physically cut off by the colonial conflict | contested shore reached by authored means | `War.VisibleToJake` | playable |
| `Main.C04.02.ThreeVerbs` | Conversation | Jake, local faction actors | contested settlement / fort approaches | complete one authored Support, Break or Liberate action; abstention remains valid but harder | route changes enough to continue pursuit | `War.Playable` plus selected qualitative war fact | playable systemic mission |
| `Main.C04.03.GlassCompass` | Object | Jake, confiscated Harlow chart, Draven courier remnants | fort/archive → ship | recover damaged family chart whose missing annotations match Ethan's shorthand | next courier lane reconstructed | `Story.HarlowChartRecovered`, `Chapter.04.Complete` | physical examine; no exposition card |

### Chapter 5 — THE LETTER (3–4 h spine)

| ID | Trigger | Participants / objects | Route | Required playable beats | Completion | Persistent facts | Presentation |
|---|---|---|---|---|---|---|---|
| `Main.C05.01.TheNavigator` | Object: sealed letter | Jake, crew, assassin observer, letter | La Liberación / port | receive and inspect the first letter naming Jake and referring to Ethan only as “the navigator” | letter authenticated | `Story.NavigatorAliasKnown` | authored letter insert |
| `Main.C05.02.HuntersKnow` | Absence / proximity | Jake, assassin network, missing contact | settlement → back route → safe room | recognize surveillance, survive/evade coordinated hunt, discover the cost of being known | assassin network exposed | `Assassins.Unlocked` | gameplay; no combat stinger |
| `Main.C05.03.FourWaysToReadASea` | Conversation | Jake, four sages as independently reachable knowledge sources | player-chosen physical routes | obtain enough environmental/navigation interpretation to decode Draven's deep-water routing | Armada deep-water corridor identified | `Story.DeepRouteKnown`, `Chapter.05.Complete` | player-paced conversations |

### Chapter 6 — THE HOLDING (4–5 h spine)

| ID | Trigger | Participants / objects | Route | Required playable beats | Completion | Persistent facts | Presentation |
|---|---|---|---|---|---|---|---|
| `Main.C06.01.FirstHolding` | WorldState | Jake, crew, potential holding actors | contested holding route | secure or receive a first defensible base through current colonial-war rules | one holding becomes functional | `Holding.FirstOwned` | playable siege/negotiation |
| `Main.C06.02.SomethingThatCanBurn` | WorldState | Jake, residents, construction/records | holding interior and approaches | restore minimum functions, choose what to protect first, establish archive/intelligence room | base can support pursuit | `Holding.IntelligenceReady` | playable; no management-overlay spectacle |
| `Main.C06.03.EthansHand` | Object | Jake, Ethan's encoded route copy | intelligence room → chart table | compare copy with family chart habits; realize Ethan is deliberately leaving tiny errors Draven will not notice | Jake proves Ethan is resisting, not serving willingly | `Story.EthanResisting`, `Chapter.06.Complete` | intimate examine / slow push |

### Chapter 7 — THE COST (5–6 h spine)

| ID | Trigger | Participants / objects | Route | Required playable beats | Completion | Persistent facts | Presentation |
|---|---|---|---|---|---|---|---|
| `Main.C07.01.TheCost` | WorldState: retaliation | Jake, crew, holding residents, attackers | holding / nearby route | respond to attack without failure screen; authored losses may persist | assault resolves | `Story.RetaliationPaid` plus authored damage/loss facts | fully playable battle |
...
| `Main.C07.03.ThePriceOfFollowing` | Conversation | Jake, core crew | Great Cabin | crew confronts Jake with accumulated costs; player listens/interjects without affinity meter | Jake commits to rescue before indiscriminate revenge | `Story.RescueBeforeRevenge`, `Chapter.07.Complete` | anchored conversation; player look retained where possible |

### Chapter 8 — THE TURN (5–6 h spine)

| ID | Trigger | Participants / objects | Route | Required playable beats | Completion | Persistent facts | Presentation |
|---|---|---|---|---|---|---|---|
| `Main.C08.01.TheArmada` | WorldState | Jake, crew, Crimson Armada scouts | open sea / occupied port | survive first undeniable Armada contact and identify its prison/navigation ship | target physically located | `Armada.Revealed` | gameplay plus authored wide |
...
| ID | Trigger | Participants / objects | Route | Required playable beats | Completion | Persistent facts | Presentation |
|---|---|---|---|---|---|---|---|
| `Main.C09.01.WhatDravenWanted` | Conversation | Jake, Ethan, core crew | La Liberación / safe shore | hear Ethan's account: Marc refused Draven access to private Harlow safe-route knowledge; Draven killed the parents and kept Ethan because he remembered the routes | motive established without supernatural retcon | `Story.DravenMotiveKnown` | player-paced cabin scene |
| `Main.C09.02.EthansGrove` | Proximity / Object | Jake, Ethan, family objects | Ethan's Grove | walk the place physically, inspect memories, allow silence; no enemy wave | remembered clearing reached | `Story.GroveVisited` | authored camera #4 boundaries, movement preserved |
...
| `Main.C09.04.WhatRageIsFor` | WorldState | Jake, Ethan | dawn after Grove → ship | Rache capability becomes available; Jake pursues Draven with a concrete Armada objective | final route locked | `Combat.RacheUnlocked`, `Story.FinalRouteKnown`, `Chapter.09.Complete` | playable departure |

### Chapter 10 — THE ARMADA (8–10 h spine)

| ID | Trigger | Participants / objects | Route | Required playable beats | Completion | Persistent facts | Presentation |
|---|---|---|---|---|---|---|---|
| `Main.C10.01.FiveWaysIn` | WorldState | Jake, Ethan, crew, war-state allies/enemies | archipelago → Armada perimeter | translate the five colonial-war end states into different access, resistance and reinforcement conditions | Armada perimeter breached | `Finale.EntryStateLocked` | large-scale gameplay |
...
| `Main.C10.03.TheBlackDeck` | Proximity | Jake, Ethan, Draven, Draven guard | flagship exterior → command deck | board flagship physically; Ethan helps where his story state permits; no teleport to boss arena | Jake reaches Draven | `Finale.DravenReached` | gameplay transition into confrontation |

...
function Test-PrivateOverlay {
    $Value = [Environment]::GetEnvironmentVariable("DARKARISEN_PRIVATE_OVERLAY")
...
$PrivateOverlay = Test-PrivateOverlay
$PublicIp = if ($PrivateOverlay) { $null } else { Require-EnvironmentValue "DARKARISEN_PUBLIC_IP" }
...
if (-not $PrivateOverlay) {
    $ConfigLines += "external-ip=$PublicIp/$LocalIp"
}
$ConfigLines += @(
    "realm=$Realm",
    "fingerprint",

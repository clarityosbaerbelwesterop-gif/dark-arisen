    ...
    const TCHAR* Transcript)
{
    ...
    Result.GoverningSource = TEXT("docs/design/npcs/named_crew_deep_dives.md; docs/design/npcs/dialogue_system.md");
...
TArray<FVoiceRoleProductionDefinition> FVoiceProductionCatalog::BuildCoreCrewRoles()
{
    return {
        Role(
            ...
            TEXT("Rich alto; subtle Fjordlund accent; direct and quiet by default, commanding only when needed; never perform grief sentimentally."),
        ...
        Role(
            ...
            TEXT("Deep baritone; loud from old forge hearing damage, warm rather than booming; Pale Isle accent worn down by years away; natural storyteller rhythm."),
        ...
        Role(
            ...
            TEXT("Cool soprano; educated precise diction; slight Rexan accent emerges only under emotion; rarely raises volume; clinical detail should sound effortless."),
        ...
        Role(
            ...
            TEXT("Warm tenor; educated diction; calm inflection; expert register switching; philosophical without sermon cadence; genuine lightness remains available."),
        ...
        Role(
            ...
            TEXT("Soft baritone; slight Caribbean/Rexan-Moran cadence; slow and deliberate without frailty; laughter like distant thunder; wisdom carried lightly."),
...
TArray<FVoiceProductionDesignGap> FVoiceProductionCatalog::BuildDesignGaps()
{
    return {
        {
            TEXT("design-gap.voice-commercial-rights"),
            TEXT("AI-generated audition output may not become shipping/Alpha-accepted voice assets until the provider's commercial-use rights, retention terms and any cost are explicitly approved and documented."),
            TEXT("SECURITY.md; CLAUDE.md Voice production")
        },
        {
            TEXT("design-gap.full-dialogue-lock"),
            TEXT("The five crew voice bibles contain many authored samples and future branches, but the complete final dialogue script with stable line IDs is not yet locked. Only the five audition lines in this catalog are shipping-text locked today."),
            TEXT("named_crew_deep_dives.md; dialogue_system.md")
        },
        {
            TEXT("design-gap.major-cast-voice-matrix"),
            TEXT("Bosses, sages, Highmoore cast and other Tier-1/Tier-2 NPCs still require the same role/direction/pronunciation/subtitle matrix before bulk voice production."),
...
bool FVoiceProductionCatalog::Validate(TArray<FString>& OutErrors)
{
    ...
    for (const FVoiceRoleProductionDefinition& Role : Roles)
    {
        ...
        {
            OutErrors.Add(FString::Printf(TEXT("Core audition line must be text/subtitle locked before synthesis: %s"), *Role.AuditionLineId));
        }
        if (Role.bAuditionGenerated || Role.bCommercialRightsApproved || !Role.ShippingAssetPath.IsEmpty())
        {
            OutErrors.Add(FString::Printf(TEXT("Source catalog may not claim generated/approved/shipping audio before external evidence exists: %s"), *Role.RoleId.ToString()));
        }
    }

    if (BuildDesignGaps().Num() != 3)
    {
        OutErrors.Add(TEXT("Voice production must preserve the current rights/script/cast gaps."));
    }

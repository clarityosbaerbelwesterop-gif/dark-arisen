...
ALaLiberacionShip::ALaLiberacionShip()
{
    ...
    ShipRoot=CreateDefaultSubobject<USceneComponent>(TEXT("ShipRoot"));SetRootComponent(ShipRoot);
    WeatherDeckRoot=CreateDefaultSubobject<USceneComponent>(TEXT("WeatherDeckRoot"));WeatherDeckRoot->SetupAttachment(ShipRoot);
    UpperDeckRoot=CreateDefaultSubobject<USceneComponent>(TEXT("UpperDeckRoot"));UpperDeckRoot->SetupAttachment(ShipRoot);
    MidDeckRoot=CreateDefaultSubobject<USceneComponent>(TEXT("MidDeckRoot"));MidDeckRoot->SetupAttachment(ShipRoot);
    HoldDeckRoot=CreateDefaultSubobject<USceneComponent>(TEXT("HoldDeckRoot"));HoldDeckRoot->SetupAttachment(ShipRoot);
    ...
    NavalCombatComponent=CreateDefaultSubobject<UNavalCombatComponent>(TEXT("NavalCombatComponent"));
    PhysicalMapClass=APhysicalMapActor::StaticClass();
}
void ALaLiberacionShip::BeginPlay(){Super::BeginPlay();ensureAlwaysMsgf(HasCompleteFourDeckStructure(),TEXT("La Liberacion requires exactly four authored deck attachment roots."));ensureAlwaysMsgf(SpawnPhysicalMap(),TEXT("La Liberacion requires a physical held map on the upper/great-cabin deck."));}
...
bool ALaLiberacionShip::CanRestInGreatCabin() const{return VoyageComponent!=nullptr&&VoyageComponent->IsJakeInGreatCabin();}
...
{
    if(IsValid(PhysicalMap))return true;UWorld* World=GetWorld();if(!World||!UpperDeckRoot||!PhysicalMapClass)return false;
    ...
    PhysicalMap=World->SpawnActor<APhysicalMapActor>(PhysicalMapClass,UpperDeckRoot->GetComponentTransform(),Parameters);if(!PhysicalMap)return false;
    PhysicalMap->AttachToComponent(UpperDeckRoot,FAttachmentTransformRules::KeepWorldTransform);return true;
}

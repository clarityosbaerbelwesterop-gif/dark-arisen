...
UENUM(BlueprintType)
enum class ENavalBroadside : uint8 { Port, Starboard };
...
/** Native ship-combat authority. Presentation may listen to fire events; damage/reload remain C++. */
UCLASS(ClassGroup=(DarkArisen), meta=(BlueprintSpawnableComponent))
class DARKARISEN_API UNavalCombatComponent : public UActorComponent
{
    GENERATED_BODY()
public:
    UNavalCombatComponent();
...
private:
    ...
    AActor* FindBroadsideTarget(ENavalBroadside Side) const;
};

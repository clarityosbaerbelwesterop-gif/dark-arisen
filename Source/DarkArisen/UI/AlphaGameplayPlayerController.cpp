...
void AAlphaGameplayPlayerController::ToggleMiniMap(){bMiniMapEnabled=!bMiniMapEnabled;}
bool AAlphaGameplayPlayerController::QuickSave(){UGameInstance* GI=GetGameInstance();auto* Story=GI?GI->GetSubsystem<UMainStorySubsystem>():nullptr;return Story&&Story->Save(AAlphaMenuPlayerController::GetAlphaSaveSlot(),0);}
bool AAlphaGameplayPlayerController::QuickLoad(){UGameInstance* GI=GetGameInstance();auto* Story=GI?GI->GetSubsystem<UMainStorySubsystem>():nullptr;if(!Story||!Story->Load(AAlphaMenuPlayerController::GetAlphaSaveSlot(),0))return false;const auto* State=Story->GetState();const FName Map=State?FMainStoryMapCatalog::ResolveMap(State->CurrentMission):NAME_None;if(Map.IsNone())return false;UGameplayStatics::OpenLevel(this,Map);return true;}
...
void AAlphaGameplayPlayerController::SetQualityPreset(int32 Level){if(auto* S=UGameUserSettings::GetGameUserSettings()){S->SetOverallScalabilityLevel(FMath::Clamp(Level,0,3));S->ApplySettings(false);S->SaveSettings();}}
void AAlphaGameplayPlayerController::ToggleVSync(){if(auto* S=UGameUserSettings::GetGameUserSettings()){S->SetVSyncEnabled(!S->IsVSyncEnabled());S->ApplySettings(false);S->SaveSettings();}}

...
bool FDarkArisenStateTreasureSiegeIntegrationSpec::RunTest(const FString& Parameters)
{
    ...
    TestTrue(TEXT("La Ciudadela config uses authored castle id"), Ciudadela->ConfigureCastle(TEXT("castle.la-ciudadela"), true));
    ...
    TestFalse(TEXT("An arbitrary treasure id cannot become state treasure canon"), Ciudadela->RecordStateTreasureRecovered(TEXT("state-treasure.fake")));
    ...
    TestTrue(TEXT("Sterling Bastion config uses authored castle id"), Sterling->ConfigureCastle(TEXT("castle.sterling-bastion"), true));

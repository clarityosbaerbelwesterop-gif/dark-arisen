...
void RequireFile(const FString& Root, const FString& Relative, TArray<FString>& Errors)
{
    ...
    {
        Errors.Add(FString::Printf(TEXT("missing alpha-content file: %s"), *Relative));
    }
}
    ...
    TArray<FString>& Errors)
{
    ...
    for (const TCHAR* Fragment : Fragments)
    {
        ...
        {
            Errors.Add(FString::Printf(TEXT("%s missing alpha-content contract: %s"), *Relative, Fragment));
        }
    }
}
}
...
int32 ValidateAlphaContentCommand(const FParsedArgs& Args)
{
    ...
    for (const FString& Relative : {
        TEXT("Source/DarkArisen/Release/AlphaContentReadiness.h"),
        TEXT("Source/DarkArisen/Release/AlphaContentReadiness.cpp"),
        TEXT("Source/DarkArisen/Release/AlphaReleaseContracts.h"),
        TEXT("Source/DarkArisen/Release/AlphaReleaseContracts.cpp"),
        TEXT("Source/DarkArisen/Tests/AlphaContentReadinessSpec.cpp")})
    {
    ...
    RequireFragments(Root, TEXT("Source/DarkArisen/Release/AlphaReleaseContracts.h"), {
        ...
        TEXT("bWindowsStartToCreditsPlaythrough"),
        TEXT("bLinuxStartToCreditsPlaythrough"),
        TEXT("bContentManifestReconciled"),
        TEXT("bPerformanceFloor60Fps"),
        TEXT("bCreditsMusicAndLicencesApproved"),
        TEXT("bFloAlphaApproval")}, Errors);

    RequireFragments(Root, TEXT("Source/DarkArisen/Tests/AlphaContentReadinessSpec.cpp"), {
        TEXT("Source cannot self-certify complete Alpha production"),
        TEXT("Three of 132 Turn identities are grounded"),
        TEXT("Auditions are not counted as shipping voice assets")}, Errors);

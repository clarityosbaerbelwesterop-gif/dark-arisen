...
void ValidateLanguagePolicy(const FString& Root, TArray<FString>& Errors)
{
    ...
    for (const FString& ScanRelative : {FString(TEXT("Source")), FString(TEXT("Tools"))})
    {
...
void ValidateGameplayContracts(const FString& Root, TArray<FString>& Errors)
{
    ...
    for (const TCHAR* Forbidden : {
        ...
        TEXT("AutosaveBeforeDecision"), TEXT("AutosaveBeforeBoss")})
    {
        if (SourceText.Contains(Forbidden, ESearchCase::CaseSensitive))
        {
            Errors.Add(FString::Printf(TEXT("forbidden gameplay API present: %s"), Forbidden));
        }
    }
}
...
{
    FString Workflows;
    for (const FString& Relative : {FString(TEXT(".github/workflows/ci.yml")), FString(TEXT(".github/workflows/deploy-streaming.yml"))})
    {
        FString Text;
    ...
    for (const TCHAR* Hosted : {TEXT("ubuntu-latest"), TEXT("windows-latest"), TEXT("macos-latest")})
    {
...
int32 ValidateCommand(const FParsedArgs& Args)
{
    ...
    UE_LOG(LogTemp, Display, TEXT("Native C++ repository validation passed for M0-M6 baseline and M7/M8 source plan."));
    return 0;
}
}
